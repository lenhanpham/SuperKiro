...
// ensureObjectSchema ensures tool schema top-level is object and cleans Kiro-rejected fields.
...
// cleanSchema recursively removes schema fields that cause Kiro 400.
func cleanSchema(m map[string]interface{}) {
	delete(m, "additionalProperties")

	// required must be non-empty array, otherwise Kiro returns Improperly formed request.
...
func KiroToOpenAIResponseWithReasoning(content, reasoningContent string, toolUses []KiroToolUse, inputTokens, outputTokens int, model, thinkingFormat string) map[string]interface{} {
	...
	if len(toolUses) > 0 {
		...
		// format thinking output according to config
		if reasoningContent != "" {
