...
type Config struct {
	...
	// ComboStickyRoundRobinLimit controls how many requests use the same model
	// before rotating in round-robin mode. Default 1 (rotate every request).
...
// GetThinkingConfig returns the thinking config
...
// UpdateThinkingConfig updates the thinking config
...
// GetPreferredEndpoint returns the preferred endpoint config
...
// UpdatePreferredEndpoint updates the preferred endpoint config
...
// GetProxyURL returns the outbound proxy URL
